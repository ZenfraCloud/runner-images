<!-- ABOUTME: What the opentofu runner image contains, how to build and smoke it locally, and its release evidence -->
<!-- ABOUTME: Tags, patch cadence and the customization guide live in the repository README -->

# zenfra-runner-opentofu

Sandbox base for Zenfra workers running OpenTofu stacks: Alpine
...
The `tofu` binary is not in the image: the worker downloads the version the
...
Its contents are currently the same as `zenfra-runner-terraform`; the two are
...
Published as `ghcr.io/zenfracloud/zenfra-runner-opentofu` for `linux/amd64` and
...
for p in amd64 arm64; do
  docker buildx build --platform linux/$p --load -t zenfra-runner-opentofu:local-$p images/opentofu
  images/opentofu/smoke.sh zenfra-runner-opentofu:local-$p linux/$p
done
```
